...
import com.ss.android.ecommerce.servicemanager.annotation.HostEcomSpiCollector
...
class HostCollectionGenerateProcessor(
    ...
    companion object {
        ...
        const val HOST_INITIALIZE_CLASS_NAME = "EcomHostServiceInitialize"
        const val HOST_INITIALIZE_INTERFACE =
            "com.ss.android.ecommerce.servicemanager.IEcomHostServiceInitialize"
    }
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        val spiCollectorClass =
            resolver.getSymbolsWithAnnotation(HostEcomSpiCollector::class.java.canonicalName)
        if (!spiCollectorClass.iterator().hasNext()) {
            return emptyList()
        }

        logger.warn("$TAG, HostCollectionGenerateProcessor start")

        val moduleName = environmentOptions["module_name"]
        val projectBuildPathList = environmentOptions["ecomProject.hostImplBuildPaths"]?.split(";")
        if (projectBuildPathList.isNullOrEmpty()) {
            throw Exception("$TAG projectBuildPathList is empty, ecommerce_saas_host_api not found")
        }
    ...
    private fun collectAllServiceInfo(
        ...
        for (path in buildPathList) {
            logger.warn("$TAG collectAllServiceInfo $path")
        }
        ...
        logger.warn("$TAG serviceNodeMap.values.size ${serviceNodeMap.values.size}")
    ...
    private fun generateServiceInitialize(serviceNodeList: List<ServiceNode>) {
        val fileSpec = FileSpec
            .builder(ImplGenerateProcessor.PACKAGE_NAME, HOST_INITIALIZE_CLASS_NAME)
            .addType(
                TypeSpec
                    .classBuilder(HOST_INITIALIZE_CLASS_NAME)
                    .addAnnotation(Keep::class)
                    .addSuperinterface(ClassName.bestGuess(HOST_INITIALIZE_INTERFACE))
